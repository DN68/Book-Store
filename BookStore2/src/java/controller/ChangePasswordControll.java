 ...
 * @author DN68
 */
@WebServlet(name = "ChangePasswordControll", urlPatterns = {"/change"})
public class ChangePasswordControll extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        acc=dao.checkPassExist(oldpass);
        if(acc ==null){
            
            request.getRequestDispatcher("view/ChangePassword.jsp").forward(request, response);
        }
        else if(!newpass.equals(renewpass)){
            request.setAttribute("mess1", "Re-newpass must match newpass");
            request.getRequestDispatcher("view/ChangePassword.jsp").forward(request, response);
        }
        else{
            dao.changepass(newpass, oldpass);
